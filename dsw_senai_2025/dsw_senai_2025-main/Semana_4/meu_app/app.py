...
class MeuFormulario(FlaskForm):
    ...
    # O validador Email() checa se o texto inserido tem um formato de e-mail válido.
...
class FormularioRegistro(FlaskForm):
    ...
    email = StringField('E-mail', validators=[DataRequired(message="O e-mail é obrigatório."), Email(message="Insira um e-mail válido.")])
    senha = PasswordField('Senha', validators=[DataRequired(message="A senha é obrigatória."), Length(min=8, message="A senha deve ter no mínimo 8 caracteres.")])
    confirmar_senha = PasswordField('Confirmar Senha', validators=[DataRequired(message="Confirme sua senha."), EqualTo('senha', message="As senhas não coincidem.")])
...
def registro():
    ...
    if form.validate_on_submit():
        nome = form.nome.data
        bio = (form.biografia.data or "").strip()
        msg = f"Bem-vindo, {nome}! Registro realizado com sucesso."
        if bio:
            msg += f" Bio: {bio[:120]}"
        flash(msg, 'success')
